...
class CommandModule:
    ...
    def __init__(self, ast, path):
        self.ast = ast
        self.path = path
        self.commands = self.parse_commands(ast)
        self.rules = {}
        self.functions = {}
        self.commands = []
    ...
    def parse_commands(self, ast):
        commands = []
        for block in ast.children:
            assert block.data == 'block'
            for child in block.children:
                if child.data == 'command':
                    commands.append(recognition.command.Command(child))
            # print(block.data, type(block.data))
        return commands
